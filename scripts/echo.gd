...
@export var playback_speed: float = 1.0 # 1.0 = same speed as player recording
...
var _settling: bool = false  # true while applying gravity after playback ends
...
func _physics_process(delta: float) -> void:
	# After playback ends, apply gravity until the echo lands.
	if _settling:
		if is_on_floor():
			_settling = false
			velocity = Vector2.ZERO
...
func _finish_playback() -> void:
	...
	# If the echo ended mid-air, let it fall to the floor.
	if not is_on_floor():
		_settling = true
		velocity.x = 0.0
...
func _apply_frame(frame: Dictionary) -> void:
	var _old_position: Vector2 = global_position
