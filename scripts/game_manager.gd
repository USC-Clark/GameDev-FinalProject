...
func _load_level(scene_path: String) -> void:
	...
	# Load TileMap from scene - separate instance from level_data load
	var packed: PackedScene = load(scene_path) as PackedScene
	if packed != null:
		var level_scene: Node = packed.instantiate()
		# Search recursively for TileMap
		var tilemap: Node = _find_tilemap(level_scene)
		if tilemap != null:
			level_scene.remove_child(tilemap)
			world.add_child(tilemap)
			print("TileMap loaded from scene: ", scene_path)
		else:
			print("WARNING - no TileMap found in: ", scene_path)
			# Print all children to debug
			for child in level_scene.get_children():
				print("  Child node: ", child.name, " type: ", child.get_class())
		level_scene.free()
	else:
		print("ERROR - could not load packed scene: ", scene_path)
...
func _find_tilemap(node: Node) -> Node:
	# Search recursively for any TileMap node
	if node is TileMap:
		return node
...
func _build_room_shell() -> void:
	# Left wall - invisible boundary only
	...
	left_rect.size = Vector2(12.0, ROOM_H)
	left_shape.shape = left_rect
	left_shape.position = left_rect.size * 0.5
	left.add_child(left_shape)

	# Right wall - invisible boundary only
	...
	right_rect.size = Vector2(12.0, ROOM_H)
	right_shape.shape = right_rect
	right_shape.position = right_rect.size * 0.5
	right.add_child(right_shape)

	# Ceiling - invisible boundary only
	...
	ceiling_rect.size = Vector2(ROOM_W, 12.0)
	ceiling_shape.shape = ceiling_rect
	ceiling_shape.position = ceiling_rect.size * 0.5
...
func _build_level_geometry(data: Dictionary) -> void:
	# platforms removed - TileMap handles all floors and walls
