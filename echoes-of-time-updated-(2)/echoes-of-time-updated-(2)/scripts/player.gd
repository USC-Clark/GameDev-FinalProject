...
func _physics_process(delta: float) -> void:
	...
	# Save a frame every physics tick during recording (Echo architecture).
	if _is_recording:
...
func _snap_to_floor() -> void:
	# The spawn point is often placed "in the air"; snap down onto the first collider.
	...
	# Our collision shapes are centered above the origin; feet are at y=0, so place origin on hit.
